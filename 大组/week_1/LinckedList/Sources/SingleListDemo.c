...
LinckedList addData(LinckedList head)       //插入结点数据
{
	LinckedList p0 = (LinckedList)malloc(LEN);
	LinckedList p1 = head;
	if (head == NULL) {
		printf("尚未创建链表！");
		printf("（回车返回）");
...
LinckedList deleteNode(LinckedList head)//删除结点
{
	LinckedList p1, p2;
	int index;
	if (head == NULL) {
...
void seekData(LinckedList head) //查找数据，关键词式查找，如ENGLISH，输入关键词LISH也可以找出
{
	char str[30] = "";
	int i, j;
	LinckedList p;
...
void destroyList(LinckedList head)//销毁链表
{
	LinckedList p;
...
void judgeLoop(LinckedList head)//判断是否成环：快慢指针法
{
	LinckedList fastP, slowP;
	fastP = slowP = head;
	while (fastP != NULL) {
		slowP = slowP->next;
...
void paritySwap(LinckedList head) //奇偶对换
{
	LinckedList p1, p2, p = head;
	while (p->next != NULL) {
		p1 = p->next;
		p2 = p1->next;
		//交换
